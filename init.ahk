...
_ShowOrMinimizeSingleProc(procStem)
{
  ...
  If (proc = "ApplicationFrameHost.exe")
    Return _ProcessUWP(A_proc, appJsonObj[app].title)
...
Hotkey, If, _IsActive("foxwq") && WinActive("> [高级房1]")
  ...
  Bind(config["4thRoom"], "FourthRoom")
...
ToggleCenter()
{
  ...
  If Not frameCacheCenter[wid]
  {
    ...
    w := keybindingConfigs.paramters.windowZoomToCenterSize.w
    h := keybindingConfigs.paramters.windowZoomToCenterSize.h
    If (w == "")
      w := 830
    If (h == "")
      h := 750
    targetWidth := width / 1680 * w
    targetHeight := height / 1050 * h
...
Bind(config["zoomToLeft"], "ZoomToLeft")
Bind(config["zoomToRight"], "ZoomToRight")
Bind(config["zoomToTopLeft"], "ZoomToTopLeft")
Bind(config["zoomToBottomLeft"], "ZoomToBottomLeft")
Bind(config["zoomToTopRight"], "ZoomToTopRight")
Bind(config["zoomToBottomRight"], "ZoomToBottomRight")
ZoomToLeft()
{
  WinRestore, A
  _WinSplitByRatio(0, 0, 0.5, 1)
}
ZoomToRight()
{
  WinRestore, A
  _WinSplitByRatio(0.5, 0, 0.5, 1)
}
ZoomToTopLeft()
{
  WinRestore, A
  _WinSplitByRatio(0, 0, 0.5, 0.5)
}
ZoomToBottomLeft()
{
  WinRestore, A
  _WinSplitByRatio(0, 0.5, 0.5, 0.5)
}
ZoomToTopRight()
{
  WinRestore, A
  _WinSplitByRatio(0.5, 0, 0.5, 0.5)
}
ZoomToBottomRight()
{
  WinRestore, A
  _WinSplitByRatio(0.5, 0.5, 0.5, 0.5)
}

; wrap built-in `WinMove` in case of `MyDockFinder` changes screen range
; consider top space occupied by mydockfinder, regardless of bottom
_WinSplitByRatio(x, y, w, h)
{
  SysGet, WA, MonitorWorkArea
  width := WARight - WALeft
  height := A_ScreenHeight - WATop
  WinMove A,, WALeft + x * width, WATop + y * height, w * width, h * height
}

; move to side/corner/center
Bind(config["moveToTopLeft"], "MoveToTopLeft")
Bind(config["moveToTop"], "MoveToTop")
Bind(config["moveToTopRight"], "MoveToTopRight")
Bind(config["moveToLeft"], "MoveToLeft")
Bind(config["moveToCenter"], "moveToCenter")
Bind(config["moveToRight"], "MoveToRight")
Bind(config["moveToBottomLeft"], "MoveToBottomLeft")
Bind(config["moveToBottom"], "MoveToBottom")
Bind(config["moveToBottomRight"], "MoveToBottomRight")

MoveToTopLeft()
{
  SysGet, WA, MonitorWorkArea
  WinMove A,, WALeft, WATop
}
MoveToTop()
{
  SysGet, WA, MonitorWorkArea
...
moveToCenter()
{
  ...
  height := A_ScreenHeight - WATop
  WinGetPos ,,, w, h, A
  WinMove A,, WALeft + (width - w) / 2, WATop + (height - h) / 2
}
MoveToRight()
{
  SysGet, WA, MonitorWorkArea
...
MoveToBottomLeft()
{
  SysGet, WA, MonitorWorkArea
  WinGetPos ,,, w, h, A
  WinMove A,, WALeft, A_ScreenHeight - h
}
MoveToBottom()
{
  SysGet, WA, MonitorWorkArea
  WinGetPos ,,, w, h, A
  WinMove A,, , A_ScreenHeight - h
}
MoveToBottomRight()
{
  SysGet, WA, MonitorWorkArea
  WinGetPos ,,, w, h, A
  WinMove A,, WARight - w, A_ScreenHeight - h
}
...
If (step == "")
  step := 100
Bind(config["moveTowardsTopLeft"], New MoveBy(-step, -step))
Bind(config["moveTowardsTop"], New MoveBy(0, -step))
Bind(config["moveTowardsTopRight"], New MoveBy(step, -step))
Bind(config["moveTowardsLeft"], New MoveBy(-step, 0))
Bind(config["moveTowardsRight"], New MoveBy(step, 0))
Bind(config["moveTowardsBottomLeft"], New MoveBy(-step, step))
Bind(config["moveTowardsBottom"], New MoveBy(0, step))
Bind(config["moveTowardsBottomRight"], New MoveBy(step, step))
...
Remap("XButton2 & WheelUp", "ShiftAltTab")
Remap("XButton2 & WheelDown", "AltTab")
...
While GetKeyState("Alt")
{
  If Not (A_proc = "ApplicationFrameHost.exe")
  {
    ...
    If (State == -1)
      Continue
    If (A_proc = "explorer.exe")
      index := _SwitchExplorer(List, index)
    WinID := List%index%
